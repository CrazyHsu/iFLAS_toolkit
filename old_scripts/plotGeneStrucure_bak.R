                      ...
                      labeltext=TRUE,labeloffset=0.4,fontsize=.7,fonttype=2,labelat="middle",...)
{
  ...
  # geneinfo = geneinfo[,seq(1,9)]
  # colnames(geneinfo) = c("chrom", "start", "end", "transcript", "score", "strand", "feature", "group", "paLen", "comparision")
...
for (i in levels(data[, 16])){
    subData <- data[data[, 16] == i, ]
    groupInfo <- subData[, 13]
    ...
    paLenData <- subData[, c(4,13,14,15)]
    ...
    p <- ggplot(paLenData, aes(x=isoform, y=paLen, label=paste0("n = ", count), fill=isoform)) +
        ...
        theme(plot.margin=unit(c(2,0,0,1), "lines"), text = element_text(size=12, face="bold"), axis.title.x=element_blank(),
...
# colnames(data) <- c("chrom", "start", "end", "transcript", "score", "strand", "feature", "group", "paLen", "comparision")
# data[]
#
#
# par(mfrow = c(2,1), mar=c(0.5,0.5,0,0), oma=c(0,0,0,0))
...
# colorbygroup <- TRUE
#
# plotGenes(data,chrom_biomart,chromstart,chromend, maxrows=50,bentline=FALSE,col="red", labeloffset=0.5,fontsize=1, bheight = 0.08, lheight = 0.4, packrow = FALSE, labelat = "start", colorbygroup=colorbygroup)
#
# maxLen <- max(ToothGrowth$len)
#
# if (is.null(colorbygroup) == FALSE){
#     ToothGrowth$dose = as.factor(ToothGrowth$dose)
#     if (length(levels(ToothGrowth$dose)) < 3){
#       colors = brewer.pal(3, "Set1")
#     }else{
#       colors = brewer.pal(length(levels(ToothGrowth$dose)), "Set1")
#     }
# }
# labelColor <- colors[ToothGrowth$dose]
# vp <- viewport(height = unit(0.5,"npc"), width=unit(0.9, "npc"),  just = c("left","top"), y = 1, x = 0)
# p <- ggplot(ToothGrowth, aes(x=dose, y=len, label=dose)) +
#     geom_violin() + geom_boxplot(width=0.1) +
#     geom_text(y=maxLen+2, vjust=0, size=4, fontface="plain", color=labelColor) +
#     coord_cartesian(ylim=c(0,maxLen), clip="off") +
#     theme(axis.title.x=element_blank(), plot.margin=unit(c(1,0,0,0), "lines"))
# print(p, vp = vp)
# plotGenes(bed,chrom_biomart,chromstart,chromend, maxrows=50,bentline=FALSE,col="red", labeloffset=0.5,fontsize=1, bheight = 0.08, lheight = 0.4, packrow = FALSE, arrowlength = 0.2, labelat = "start", colorbygroup=TRUE)